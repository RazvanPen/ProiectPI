...
public class LogInFrame extends JFrame implements ActionListener, MouseListener{
	...
	private ImageIcon librarian = new ImageIcon(new ImageIcon("poza3.png").getImage().getScaledInstance(450, 685, Image.SCALE_SMOOTH));
	private ImageIcon catLogo = new ImageIcon(new ImageIcon("poza4.png").getImage().getScaledInstance(400, 150, Image.SCALE_SMOOTH));
	private ImageIcon bspeech = new ImageIcon (new ImageIcon("poza5.png").getImage().getScaledInstance(300, 300, Image.SCALE_SMOOTH));
	private JLabel leftImg = new JLabel();
 	private JLabel username = new JLabel("Utilizator");
	private JLabel password = new JLabel("Parola");
	private JLabel welcomemess = new JLabel();
	private JPanel logInP = new JPanel();
	private JButton login = new JButton("Logheaza-te");
	...
	private JLabel newacc = new JLabel("Nu aveti inca un cont? Creeaza-ti imediat unul!");
		...
		if(e.getSource() == login){
			try {
				...
				if(!res.next()) {
					...
					Class.forName("com.mysql.cj.jdbc.Driver");
					Connection con1 = DriverManager.getConnection("jdbc:mysql://localhost:3306/flyscan", "root", "razvan05");
					Statement stmt1 = con.createStatement();
					ResultSet res1 = stmt.executeQuery("SELECT * FROM zboruri");
					while(res1.next()) {
						
						int idZboruri = res1.getInt("idZboruri");
						String Plecare = res1.getString("Plecare");
						String Destinatie = res1.getString("Destinatie");
						String Companie = res1.getString("Companie");
						int Pret = res1.getInt("Pret");
						String Ora = res1.getString("Ora");
						System.out.println(idZboruri+ " " + Plecare + " "+Destinatie+" "+Companie+" "+Pret+" "+Ora+"\n");
	...
	public LogInFrame(){
		...
		newacc.setBounds(115, 330, 350, 50);
